...
- [Midnight Private Auction](https://github.com/pplmaverick/midnight-private-auction) - Sealed-bid auction with ZK commit-reveal: bid amounts stay private via Compact's private state during bidding and are revealed only when bidders call `revealBid()` after close. Includes `startingPrice` enforcement and `finalizeAuction` for no-sale settlement. Deployed on Midnight mainnet.
